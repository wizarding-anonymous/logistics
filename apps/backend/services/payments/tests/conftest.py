...
from payments.database import Base, get_db
...
def create_test_token(user_id: uuid.UUID, org_id: uuid.UUID) -> str:
    claims = {
        "sub": str(user_id),
        "org_id": str(org_id),
...
# --- Data Fixtures ---
@pytest.fixture(scope="function")
async def test_invoice(test_db: async_sessionmaker) -> Invoice:
    """Fixture to create a test invoice."""
    async with test_db() as session:
        invoice = Invoice(
            order_id=uuid.uuid4(),
